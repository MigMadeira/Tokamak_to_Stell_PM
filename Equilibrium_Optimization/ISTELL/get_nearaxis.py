...
vmec = Vmec(os.path.join(current_path, 'results', f'wout_ISTELL_final.nc'))
# vmec = Vmec(os.path.join(current_path, 'results', f'input.ISTELL_final'))
# vmec.indata.mpol            = 8
# vmec.indata.ntor            = 8
...
b1.run()
# b1.bx.write_boozmn(os.path.join(current_path, 'results', "boozmn_ISTELL_final.nc"))
if plot_boozer:
    print("Plot BOOZ_XFORM")
    fig = plt.figure()
    bx.surfplot(b1.bx, js=1, fill=False, ncontours=35)

    # Get the current axes
    ax = plt.gca()

    # Set x-axis label and tick label font sizes
    ax.set_xlabel(r"Boozer toroidal angle $\phi$", fontsize=18)
    ax.set_xticklabels(ax.get_xticklabels(), fontsize=16)

    # Set y-axis label and tick label font sizes
    ax.set_ylabel(r"Boozer poloidal angle $\theta$", fontsize=18)
    ax.set_yticklabels(ax.get_yticklabels(), fontsize=16)

    title = ax.title
    title.set_fontsize(18)
    
    plt.savefig(os.path.join(current_path,'results','get_nearaxis_plots', "Boozxform_surfplot_1_ISTELL_final.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
    ...
    plt.savefig(os.path.join(current_path, 'results','get_nearaxis_plots', "Boozxform_surfplot_2_ISTELL_final.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
    ...
    plt.savefig(os.path.join(current_path, 'results','get_nearaxis_plots', "Boozxform_surfplot_3_ISTELL_final.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
    
    fig = plt.figure(); bx.symplot(b1.bx, helical_detail = False, sqrts=True)
    plt.savefig(os.path.join(current_path, 'results','get_nearaxis_plots', "Boozxform_symplot_ISTELL_final.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
    
    fig = plt.figure(); bx.modeplot(b1.bx, sqrts=True); plt.xlabel(r'$s=\psi/\psi_b$')
    plt.savefig(os.path.join(current_path, 'results','get_nearaxis_plots', "Boozxform_modeplot_ISTELL_final.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
...
for jmn in range(len(b1.bx.xm_b)):
    ...
    if m==2:
        ...
        if doplot:
            plt.plot(np.sqrt(s_fine), np.polyval(p, s_fine),'r')
plt.savefig(os.path.join('results','get_nearaxis_plots','ISTELL_final_nearaxis_fit.pdf'))
# if show:
#     plt.show()
# plt.close()
...
print(stel.rc)
print(stel.zs)
print(rf"R_0(\phi) = {stel.rc[0]} + {stel.rc[1]}cos(2\phi) + {stel.rc[2]}cos(4\phi)")
print(rf"z_0(\phi) = {stel.zs[0]} + {stel.zs[1]}sin(2\phi) + {stel.zs[2]}sin(4\phi)")
...
plt.ylabel(r'$B_2$')
plt.legend()
plt.savefig('ISTELL_final_B0B1B2.pdf')
plt.legend()
...
iota0  = stel.iota
fig = plt.figure(figsize=(6.153846, 4.61538))
...
plt.title(f'B0={BBar:.2f}, etabar={eta_bar:.2f}, B2c={np.mean(B2c):.2f}, nfp={vmec.wout.nfp}',fontsize=18)
plt.tight_layout()
plt.savefig('nearaxis_ISTELL_final.pdf')
