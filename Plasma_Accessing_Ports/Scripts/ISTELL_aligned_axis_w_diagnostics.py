...
# Set some parameters
nphi = 64*4  # need to set this to 64 for a real run. We multiply by 2*nfp=4 to keep the same resolution as when a hfp is used.
ntheta = 64  
Plot = False

input_name = 'wout_ISTELL_final.nc'
coordinate_flag = 'cartesian'
famus_filename = '../Data/ISTELL_aligned_full_2cm.focus'
...
#PermanentMagnetGrid uses the nfp and stellsym of the given surface
s.nfp = 1
s.stellsym = False
...
#Import the port data from a csv
diagnostic_data = pd.read_csv("DiagnosticTracking.csv")

cylinder_list = []

#set the tolerance around the ports
tol = 0.01

#create the ports modelled as cylinders
for i in range(len(diagnostic_data["h_port"])):
    cylinder_list.append(sopp.Cylinder(0.46, 0.094, diagnostic_data["tor"][i], diagnostic_data["pol"][i], 
                                       diagnostic_data["h_total"][i] + tol, diagnostic_data["Outer_r"][i] + tol, 
                                       diagnostic_data["theta"][i], diagnostic_data["phi"][i]))

if Plot:
    for i in range(len(cylinder_list)):
        cylinder_to_vtk(cylinder_list[i], f"../Plots/diagnostics_ISTTOK/diagnostic {i}")
...
save_plots = False
if save_plots:
    # Save the MSE history and history of the m vectors. WARNING:May have gigabytes, be careful
    #np.savetxt(
